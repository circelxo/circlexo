...
<div x-data="{ dropdownOpen: false }" :class="{ 'block z-50 w-auto dark:bg-zinc-900 dark:border-zinc-800' : open, 'hidden': ! open }" class="relative flex-shrink-0 sm:p-0 dark:text-zinc-200 sm:flex sm:w-auto sm:bg-transparent sm:items-center" x-cloak>
    <button @click="dropdownOpen=!dropdownOpen" class="flex p-2.5 lg:p-2 w-full gap-1 text-[13px] hover:bg-zinc-200/70 rounded-lg justify-between items-center hover:text-black dark:hover:text-zinc-100 dark:hover:bg-zinc-700/60  overflow-hidden group-hover:autoflow-auto items">
        <span class="relative flex items-center gap-2">
            <x-avatar src="{{ auth('accounts')->user()->avatar() }}" srcset="{{ auth('accounts')->user()->avatar() }}" alt="{{ auth('accounts')->user()->name }} photo" size="xs" />
            <span @class([
                'flex-shrink-0 ease-out duration-50',
                'hidden' => ($position != 'bottom')
            ])>{{ auth('accounts')->user()->name }}</span>
        </span>
        <svg :class="{ 'rotate-180' : '{{ $position }}' == 'bottom' }" class="relative ltr:right-0 rtl:left-0 w-4 h-4 ease-out  ltr:-translate-x-0.5 rtl:translate-x-0.5 fill-current group-hover:delay-150 duration-0 group-hover:duration-300" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clip-rule="evenodd" /></svg>
    </button>
    <div wire:ignore x-show="dropdownOpen" @mouse.leave="dropdownOpen=false" @click.away="dropdownOpen=false" x-transition:enter="transition ease-out duration-100" x-transition:enter-start="transform opacity-0 sm:scale-95" x-transition:enter-end="transform opacity-100 sm:scale-100" x-transition:leave="transition ease-in duration-75" x-transition:leave-start="transform opacity-100 sm:scale-100" x-transition:leave-end="transform opacity-0 sm:scale-95"
        @class([
            'z-50',
            'ltr:left-0 rtl:right-0 absolute w-full bottom-0 sm:origin-bottom mb-12' => ($position == 'bottom'),
            'top-0 sm:origin-top ltr:right-0 rtl:left-0 ltr:mr-5 rtl:ml-5 mt-14 w-full max-w-xs fixed' => ($position != 'bottom')
        ])
        x-cloak>
        <div class="pt-0 mt-1 bg-white border dark:border-zinc-700 text-zinc-600 dark:text-white/70 dark:bg-zinc-900 dark:shadow-xl sm:gap-0.5 sm:border shadow-md rounded-xl border-zinc-200/70 dark:border-white/10">
            <div class="py-3.5">
                <div class="px-[18px] text-[13px] font-bold text-ellipsis overflow-hidden whitespace-nowrap">{{ auth('accounts')->user()->name }}</div>
                <div class="px-[18px] text-[13px] text-ellipsis overflow-hidden whitespace-nowrap">{{ auth('accounts')->user()->email }}</div>
            </div>

            <div class="w-full h-px mb-2 bg-slate-100 dark:bg-zinc-700"></div>
